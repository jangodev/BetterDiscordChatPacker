...
r = Fore.LIGHTRED_EX
token = 'enter discord token here as well as in the token.txt file'
...
def tokenchecker():
    ...
    if response2.status_code == 200:
            print(f'{w}[{g}+{w}] » Token {g}{token}{w} works')
...
start = f"its over for u <@{id_to_pack}> :rofl:"
data1 = {"content": start}
sentences = random.choices(lines, k=min(len(lines), 900))
...
for sentence in sentences:
    ...
    #new v
    print(f"Sending: '{modified_sentence}' (Length: {len(modified_sentence)})")
